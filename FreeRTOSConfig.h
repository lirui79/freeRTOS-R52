...
// 必须指定定时器服务任务的优先级 (0 ~ configMAX_PRIORITIES-1)
// 建议设置为较高优先级，以确保定时器回调及时执行
#define configTIMER_TASK_PRIORITY    ( configMAX_PRIORITIES - 1 )

// 必须指定定时器命令队列的长度
// 如果队列太短，xTimerStart/Reset 可能会失败
#define configTIMER_QUEUE_LENGTH     10 

// 必须指定定时器服务任务的堆栈深度 (单位: 字/Word, 不是字节)
// 根据回调函数的复杂度调整，通常 200-500 足够
