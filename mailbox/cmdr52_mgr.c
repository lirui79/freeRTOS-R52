...
int             cmdr52_mgr_init(cmdr52_mgr_t *mgr) {
    ...
    for (i = 0; i < mgr->vtb_size; ++i) {
        cmdr52_session_init(&mgr->vtb[i], sessionID + i);
    }
...
cmdr52_session_t*    cmdr52_mgr_get_session(uint32_t sessionID) {
    ...
    return &cmdr52_mgr_get()->vtb[sessionID];
}
...
static uint32_t cmd_check(cmdMsg_t *cmdMsg, cmdr52_session_t **session)
{
    ...
    if (r52ID != cmdr52_mgr_get()->r52coreID) {
        retCode = CMD_ERR_INVALID_SESSIONID;
        goto RETURN_ERROR;
    }

    *session = cmdr52_mgr_get_session(sesID);
    if (*session == NULL) {
        retCode = CMD_ERR_INVALID_SESSIONID;
        goto RETURN_ERROR;
    }
...
RETURN_ERROR:
    {
        ...
        cmdr52_session_t *session1 = cmdr52_mgr_get_session(0);
        cmd_init(cmdMsg1);
...
int32_t           cmdr52_mgr_wait_cmdMsg(cmdMsg_t *cmdMsg, uint32_t mgrid) {
    ...
    vcmd_mgr = cmdr52_get_vcmd_mgr(mgrid);
